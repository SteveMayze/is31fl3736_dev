...
#define MASTER_DISPLAY Addr_GND_GND
#define SLAVE1_DISPLAY Addr_GND_SCL
#define SLAVE2_DISPLAY Addr_GND_SDA
...
#define ABM_T1_0021 (T1_0021<<T1_BITS) // 0.21s
#define ABM_T1_0042 (T1_0042<<T1_BITS) // 0.42s
#define ABM_T1_0084 (T1_0084<<T1_BITS) // 0.84s
#define ABM_T1_0168 (T1_0168<<T1_BITS) // 1.68s
#define ABM_T1_0336 (T1_0336<<T1_BITS) // 3.36s
#define ABM_T1_0672 (T1_0672<<T1_BITS) // 6.72s
#define ABM_T1_1344 (T1_1344<<T1_BITS) // 13.44s
#define ABM_T1_2688 (T1_2688<<T1_BITS) // 26.88s

#define ABM_T2_00000 (T2_00000<<T2_BITS) // 0.21s
#define ABM_T2_00021 (T2_00021<<T2_BITS) // 0.21s
#define ABM_T2_00042 (T2_00042<<T2_BITS) // 0.42s
#define ABM_T2_00084 (T2_00084<<T2_BITS) // 0.84s
#define ABM_T2_00168 (T2_00168<<T2_BITS) // 1.68s
#define ABM_T2_00336 (T2_00336<<T2_BITS) // 3.36s
#define ABM_T2_00672 (T2_00672<<T2_BITS) // 6.72s
#define ABM_T2_01344 (T2_01344<<T2_BITS) // 13.44s
#define ABM_T2_02688 (T2_02688<<T2_BITS) // 26.88s

#define ABM_T3_0021 (T3_0021<<T3_BITS) // 0.21s
#define ABM_T3_0042 (T3_0042<<T3_BITS) // 0.42s
#define ABM_T3_0084 (T3_0084<<T3_BITS) // 0.84s
#define ABM_T3_0168 (T3_0168<<T3_BITS) // 1.68s
#define ABM_T3_0336 (T3_0336<<T3_BITS) // 3.36s
#define ABM_T3_0672 (T3_0672<<T3_BITS) // 6.72s
#define ABM_T3_1344 (T3_1344<<T3_BITS) // 13.44s
#define ABM_T3_2688 (T3_2688<<T3_BITS) // 26.88s

#define ABM_T4_00000 (T4_00021<<T4_BITS) // 0.21s
#define ABM_T4_00021 (T4_00021<<T4_BITS) // 0.21s
#define ABM_T4_00042 (T4_00042<<T4_BITS) // 0.42s
#define ABM_T4_00084 (T4_00084<<T4_BITS) // 0.84s
#define ABM_T4_00168 (T4_00168<<T4_BITS) // 1.68s
#define ABM_T4_00336 (T4_00336<<T4_BITS) // 3.36s
#define ABM_T4_00672 (T4_00672<<T4_BITS) // 6.72s
#define ABM_T4_01344 (T4_01344<<T4_BITS) // 13.44s
#define ABM_T4_02688 (T4_02688<<T4_BITS) // 26.88s
#define ABM_T4_05376 (T4_05376<<T4_BITS) // 26.88s
#define ABM_T4_10752 (T4_10752<<T4_BITS) // 26.88s


#define IS31FL3736_CONFIG_REG        0x00

#define IS31FL3736_SSD_BITS          0x00
#define IS31FL3736_NORMAL_OPERATION  0x01
#define IS31FL3736_SOFTARE_SHUTDOWN  0x00

#define IS31FL3736_B_EN_BITS         0x01
#define IS31FL3736_ABM_MODE          0x01
#define IS31FL3736_PWM_MODE          0x00

#define IS31FL3736_OSD_BITS          0x02

#define IS31FL3736_SYNC_BITS         0x06
#define IS31FL3736_SYNC_MASTER       0x01
#define IS31FL3736_SYNC_SLAVE        0x02
#define IS31FL3736_SYNC_OFF          0x00
...
void IS31FL3736_abm_init(uint8_t addr)//white LED
{
  uint8_t i;
  IS_IIC_WriteByte(addr, 0xFE, 0xc5); //Unlock FDh
  IS_IIC_WriteByte(addr, 0xFD, 0x00); // Turn to page 0: LED Control
  for(i=0;i<0x18;i++) {
    IS_IIC_WriteByte(addr, i, 0xFF); // Enable all LEDs
  }
  ...
  IS_IIC_WriteByte(addr,0xFE,0xc5);//unlock FDh
  IS_IIC_WriteByte(addr,0xFD,0x02);//Turn to page 2: ABM registers
  uint8_t pwmAddr = 0;
  for ( uint8_t i=0; i<12; i++) {
    for( uint8_t j=0; j<16; j=j+2) {
      IS_IIC_WriteByte(addr, pwmAddr, abm_reg[i][j]);//Set PWM data to 0
      pwmAddr = pwmAddr+2;
    }
  }

  IS_IIC_WriteByte(addr,0xFE,0xc5);//unlock FDh
  IS_IIC_WriteByte(addr,0xFD,0x03);//Turn to page 3: Config Registers
  IS_IIC_WriteByte(addr,0x00,0x00); //Enable software shutdown

  uint8_t timing_1 = ABM_T1_0084|ABM_T2_00000;
  uint8_t timing_2 = ABM_T3_0084|ABM_T4_00084;
  // uint8_t timing_3 = ABM_T1_2688|ABM_T2_00021;
  // uint8_t timing_4 = ABM_T3_0021|ABM_T4_02688;
  IS_IIC_WriteByte(addr, IS31FL3736_ABM_1_1, timing_1);
  IS_IIC_WriteByte(addr, IS31FL3736_ABM_1_2, timing_2);
  IS_IIC_WriteByte(addr, IS31FL3736_ABM_1_3, (ABM_LE_T3<<ABM_LE_BIT)|(ABM_LB_T1<<ABM_LB_BIT)|(ABM_LTA_ENDLESS<<ABM_LTA_BIT));
  IS_IIC_WriteByte(addr, IS31FL3736_ABM_1_4, 0b00000000);

  IS_IIC_WriteByte(addr, IS31FL3736_ABM_2_1, timing_1);
  IS_IIC_WriteByte(addr, IS31FL3736_ABM_2_2, timing_2);
  IS_IIC_WriteByte(addr, IS31FL3736_ABM_2_3, (ABM_LE_T3<<ABM_LE_BIT)|(ABM_LB_T4<<ABM_LB_BIT)|(ABM_LTA_ENDLESS<<ABM_LTA_BIT));
  IS_IIC_WriteByte(addr, IS31FL3736_ABM_2_4, 0b00000000);

  IS_IIC_WriteByte(addr, IS31FL3736_ABM_3_1, timing_1);
  IS_IIC_WriteByte(addr, IS31FL3736_ABM_3_2, timing_2);
  IS_IIC_WriteByte(addr, IS31FL3736_ABM_3_3, (ABM_LE_T3<<ABM_LE_BIT)|(ABM_LB_T2<<ABM_LB_BIT)|(ABM_LTA_ENDLESS<<ABM_LTA_BIT));
  IS_IIC_WriteByte(addr, IS31FL3736_ABM_3_4, 0b00000000);
...
void setup()
{
  ...
  IS31FL3736_abm_init(SLAVE1_DISPLAY);
  IS_IIC_WriteByte(SLAVE1_DISPLAY, 0xFE, 0xc5); //Unlock FDh
  IS_IIC_WriteByte(SLAVE1_DISPLAY, 0xFD, 0x03); //Turn to page 3: function registers
  IS_IIC_WriteByte(SLAVE1_DISPLAY, 0x01, 0x1f); //global current
  uint8_t config = 0x00;
  config = (IS31FL3736_SYNC_SLAVE<<IS31FL3736_SYNC_BITS)  |
           (IS31FL3736_ABM_MODE<<IS31FL3736_B_EN_BITS) |
           (IS31FL3736_NORMAL_OPERATION<<IS31FL3736_SSD_BITS);
  IS_IIC_WriteByte(SLAVE1_DISPLAY, IS31FL3736_CONFIG_REG, config ); // 0b10000011
  IS_IIC_WriteByte(SLAVE1_DISPLAY, IS31FL3736_TIME_UPDATE ,0x00);

  IS31FL3736_abm_init(SLAVE2_DISPLAY);
  IS_IIC_WriteByte(SLAVE2_DISPLAY, 0xFE, 0xc5); //Unlock FDh
  IS_IIC_WriteByte(SLAVE2_DISPLAY, 0xFD, 0x03); //Turn to page 3: function registers
  IS_IIC_WriteByte(SLAVE2_DISPLAY, 0x01, 0x1f); //global current
  config = 0x00;
  config = (IS31FL3736_SYNC_SLAVE<<IS31FL3736_SYNC_BITS)  |
           (IS31FL3736_ABM_MODE<<IS31FL3736_B_EN_BITS) |
           (IS31FL3736_NORMAL_OPERATION<<IS31FL3736_SSD_BITS);
  IS_IIC_WriteByte(SLAVE2_DISPLAY, IS31FL3736_CONFIG_REG, config ); // 0b10000011
  IS_IIC_WriteByte(SLAVE2_DISPLAY, IS31FL3736_TIME_UPDATE ,0x00);


  IS31FL3736_abm_init(MASTER_DISPLAY);
  //can use buffer write type as figure 7 in datasheet
  IS_IIC_WriteByte(MASTER_DISPLAY, 0xFE, 0xc5); //Unlock FDh
  IS_IIC_WriteByte(MASTER_DISPLAY, 0xFD, 0x03); //Turn to page 3: function registers
  IS_IIC_WriteByte(MASTER_DISPLAY, 0x01, 0x1f); //global current
  config = 0x00;
  config = (IS31FL3736_SYNC_MASTER<<IS31FL3736_SYNC_BITS) |
           (IS31FL3736_ABM_MODE<<IS31FL3736_B_EN_BITS) |
           (IS31FL3736_NORMAL_OPERATION<<IS31FL3736_SSD_BITS);
  IS_IIC_WriteByte(MASTER_DISPLAY, IS31FL3736_CONFIG_REG, config);// 0b01000011
  IS_IIC_WriteByte(MASTER_DISPLAY, IS31FL3736_TIME_UPDATE, 0x00);
